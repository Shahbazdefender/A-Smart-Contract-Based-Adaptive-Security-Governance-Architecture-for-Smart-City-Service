...
# time.sleep(0.015)# for 15ms  3000 
	...
	ServiceLoad=random.uniform(0.06,0.08)
	time.sleep(ServiceLoad)# for 600ms  100
	collaborative(n1)
...
#################################################TestBed Timmer################################
	...
	#    print('Received-2 from server: ' + data+''+str(node))  # show in terminal
		p1 = Thread(target = one(node))
		p2 = Thread(target = two(node))
		p3 = Thread(target = three(node))
