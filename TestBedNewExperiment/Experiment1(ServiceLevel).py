...
# time.sleep(0.015)# for 15ms  3000 
        ...
        time.sleep(sleep+ServiceLoad-0.04)
        recieve_time_seconds = time.time()
...
#        client_socket.send(message.encode())  # 1.Send Txid of Blockchain
      #  data = client_socket.recv(2024).decode()  # 2.receive Ack From Server 
        ...
        sleep=ServiceLoad+1.2
      ##################################We take This snapshot as throughput of system(Start)#######################################
        time.sleep(sleep)# for 600ms  100
 
        recieve_time = now.strftime("%H:%M:%S")  
        recieve_time_seconds = time.time()
          #print(list3b)
        
        tuple_ind2 = (str(current_time1), str(recieve_time), str(((recieve_time_seconds - current_time_seconds1))))
